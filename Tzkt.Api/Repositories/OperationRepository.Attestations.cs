...
namespace Tzkt.Api.Repositories
{
    public partial class OperationRepository
    {
        ...
        {
            var sql = new SqlBuilder(@"SELECT COUNT(*) FROM ""AttestationOps""")
                .Filter("Level", level)
        ...
        public async Task<IEnumerable<AttestationOperation>> GetAttestations(string hash, Symbols quote)
        {
            ...
            return rows.Select(row => new AttestationOperation
            {
                Id = row.Id,
                Level = row.Level,
                Block = row.Hash,
        ...
        public async Task<IEnumerable<Activity>> GetAttestationOpsActivity(
            ...
            Symbols quote)
        {
            ...
            {
                if (account is not RawDelegate baker || baker.AttestationsCount == 0)
                    continue;
            ...
            var or = new OrParameter(("DelegateId", ids));
